...
// import uploads from '../uploads/userUpload.js'
    ...
    if(req.file){ 
        try{
            ...
            if(result){
                ...
                FileSystem.arguments(`uploads/${req.file.filename}`)
            }
        }catch(e){
            return next(
...
const getProfile = async (req , res) => {
    try{
     ...
     res.status(200).json({
       ...
       return next(new AppError('Failed to fetch profile detailed' , 500));
    }
};

const isLoggedIn = async(req,res) => {
    console.log('Tera baap mein hoon');
}
...
const forgotPassword = async (req,res, next) => {
    ...
    const message = `You can reset your password by clicking <a href=${resetPasswordURL} target="_blank"> Reset your password</a>\n If the above link does not work for some reason then copy past this link in new tab ${resetPasswordURL}.\n If you have not requested this , kindly ignore this`;
